...
Upgrades existing system with selective client control, smart aggregation, and version control
Integrates with existing flask_app_advanced.py without breaking current functionality
"""
...
def integrate_with_existing_app(app, client_manager, aggregation_manager):
    ...
    def select_clients_for_training():
        ...
        try:
            ...
            if client_ids:
                # Manual selection
                client_manager.set_selected_clients(client_ids)
            else:
                # Automatic selection
                selected = client_manager.select_clients_for_training(num_clients, selection_method)
                client_manager.set_selected_clients(selected)
            ...
            return jsonify({
                ...
                'total_eligible': len([
                    cid for cid in client_manager.registered_clients.keys()
                    if cid not in client_manager.blocked_clients
                ])
            })
    ...
    def block_client():
        ...
        try:
            ...
            success = client_manager.block_client(client_id, reason)
            if success:
                return jsonify({
    ...
    def unblock_client():
        ...
        try:
            ...
            success = client_manager.unblock_client(client_id)
            if success:
                return jsonify({
    ...
    def force_aggregation():
        ...
        try:
            return jsonify(aggregation_manager.force_aggregation())
        except Exception as e:
            return jsonify({'error': str(e)}), 400
    ...
    def rollback_model():
        ...
        try:
            ...
            return jsonify(aggregation_manager.rollback_to_version(version))
    ...
    def get_model_versions():
        ...
        try:
            ...
            return jsonify({
                ...
                'versions': aggregation_manager.get_version_history(limit),
    ...
    def get_aggregation_logs():
        ...
        try:
            ...
            return jsonify({
                'logs': aggregation_manager.get_aggregation_logs(limit),
                'rejected_updates': aggregation_manager.get_rejected_updates(limit)
            })
        except Exception as e:
            return jsonify({'error': str(e)}), 400
...
def main():
    ...
    try:
        ...
        print("\n" + "="*80)
        print("🔥 ENHANCED SERVER FEATURES ACTIVATED")
        print("="*80)
        print("\n📊 SERVER RUNNING ON:")
        ...
        print("   9. Select Clients: POST /api/client-management/select")
        print("   10. Block Client: POST /api/client-management/block")
        print("   11. Unblock Client: POST /api/client-management/unblock\n")
        
        print("   🔄 SMART AGGREGATION:")
        print("   12. Queue Status: GET /api/aggregation/queue-status")
        print("   13. Force Aggregation: POST /api/aggregation/force")
        print("   14. Aggregation Logs: GET /api/aggregation/logs")
        print("   15. Model Versions: GET /api/model/versions")
        print("   16. Rollback: POST /api/model/rollback\n")
        
        print("   📈 MANAGEMENT DASHBOARD:")
        print("   17. Enhanced Dashboard: GET /api/management/dashboard")
        print("   18. Client Statistics: Integrated in management dashboard")
        print("   19. Aggregation Monitoring: Real-time queue status\n")
